...
interface ResultMessageProps {
  multiGameState: MultiGameState | undefined;
  userId: string | undefined;
}

export default function RoundResultMulti({ multiGameState, userId }: ResultMessageProps) {
  ...
  const [song, setSong] = useState<string | undefined>(multiGameState?.currentRound.songName);

  const userResult = multiGameState?.currentRound.results.find(
    (result) => result.userId === userId,
  );
  useEffect(() => {
    if (multiGameState?.status === MultiLobbyStatus.Revealing) {
      setSong(multiGameState.currentRound.songName);
      setScore(userResult?.score ?? 0);
    }
  }, [multiGameState]);

  const show = multiGameState?.status === MultiLobbyStatus.Revealing;
  return (
    <div
